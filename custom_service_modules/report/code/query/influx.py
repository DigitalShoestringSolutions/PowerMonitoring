...
def total_energy(config, dt_1, dt_2, dt_3):
    query = f"""
            from(bucket: _bucket)
                |> range(start: _start, stop: _stop)
                |> filter(fn: (r) => r["_measurement"] == "energy")
        ...
        while True:
            try:
                async with InfluxDBClientAsync(url=url, token=token, org=org) as client:
                    ...
                    data_frame = await query_api.query_data_frame(query, params=params)
                ...
                if attempt_count <= 3:
                    logger.warning(
                        f"Retrying due to timeout during influx query - attempt {attempt_count}"
                    )
                else:
                    raise
